...
#define IS_TEXT(file_name) (strlen(file_name) >= 4 &&file_name[strlen(file_name)-4] == '.' && file_name[strlen(file_name)-3] == 't' && file_name[strlen(file_name)-2] == 'x' && file_name[strlen(file_name)-1] == 't' ? 1 : 0)
        ...
        if (FD_ISSET(udpfd, &rset)) {
            ...
            while(1){
                ...
                if (dir == NULL) {
                    ...
                    while ((entry = readdir(dir)) != NULL ) {
                        if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0 || !IS_TEXT(entry->d_name)) {
                            continue;
                        }
                        ...
                        if (fd < 0) {
                            ...
                            while ((bytes_read = read(fd, buffer, DIM_BUFF)) > 0) {
                                for ( i = 0; i < bytes_read; ++i ) {
                                    ...
                                    if (c == '\n') {
                                        if (count >= invioUDP.numOccorrenze && valid ) {
                                            risultato++;
                                        }
                                        count = 0;
                                        //is_new_line = 1;
                            ...
                            // Verifica ultima riga se non finisce con \n
                            if (count >= invioUDP.numOccorrenze && valid) {
                                risultato++;
                            }
