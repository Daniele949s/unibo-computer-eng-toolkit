...
public class RMI_Server extends UnicastRemoteObject implements RMI_interfaceFile {
	...
	//Elimina tutti i caratteri numerici in un file di testo
	@Override
			...
			try (BufferedReader reader = new BufferedReader(new FileReader(originalFile))) {
				...
				try (BufferedWriter writer = new BufferedWriter(new FileWriter(tempFile))) {
					...
					while ((line = reader.readLine()) != null) {
						String[] caratteri = line.split("");
						String numeri = "0123456789";
						//String risultato = input.replaceAll("\\d", "");		//ci4o -> cio
						for(int i=0; i < caratteri.length; i++){
							if (!numeri.contains(caratteri[i])) { // Condizione di scrittura
								writer.write(caratteri[i]);
							} else {
								result ++;
							}
						}
