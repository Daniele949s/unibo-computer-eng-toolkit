...
#define IS_TEXT(file_name) (strlen(filename) >= 4 && file_name[strlen(file_name)-4] == '.' && file_name[strlen(file_name)-3] == 't' && file_name[strlen(file_name)-2] == 'x' && file_name[strlen(file_name)-1] == 't' ? 1 : 0)
...
int main(int argc, char **argv)
{
    ...
    for (;;)
    {
        ...
        /*trasferimento di tutti i file binari (quindi che non hanno estensione finale .txt)di una directory (e dei suoi sottodierettori di 1 livello)
         passata dall utente  all utente */
                    ...
                    // Directory esiste, la scorro
                        ...
                        while ((entry = readdir(dir)) != NULL) {
                            ...
                            if(entry->d_type != DT_DIR){
                                ...
                                //oppure uso strncmp(fileNamefinale, suffisso, strlen(suffisso)) == 0  qui sotto, che se è file.txt lo skippo

                                if (entry -> d_name[0] == '.' || IS_TEXT(entry->d_name)){
                                    continue;
                                }
                                ...
                                snprintf(dirpath, sizeof(dirpath), "%s/%s", directory, entry->d_name);
                                ...
                                while ((subentry = readdir(subdir)) != NULL) {
                                    if (strcmp(subentry->d_name, ".") != 0 && strcmp(subentry->d_name, "..") != 0 && !IS_TEXT(subentry->d_name)) {
                                        ...
                                        // Invio nome file
                                        if (write(connfd, subentry -> d_name, MAX_NAME_SIZE) < 0) {
                                            perror("write");
                                            close(fd);
                                            closedir(subdir);
                                            close(connfd);
        ...
        /*contare num delle linee che iniziano con lettera maiuscola e contengono uno specifico num di 
        occorrenze di un char indicato da utente, all interno di tuttti i file di testo sul direttorio */
            ...
            while(1){
                ...
                if (dir == NULL) {
                    ...
                    while ((entry = readdir(dir)) != NULL ) {
                        ...
                        if (entry -> d_name[0] == '.' || strcmp(entry->d_name, "..") == 0 || !IS_TEXT(entry->d_name) ){
                            continue;
                        }
                        //oppure uso strncmp(fileNamefinale, suffisso, strlen(suffisso)) != 0  
                        ...
                        if (fd < 0) {
                            ...
                            while ((bytes_read = read(fd, buffer, DIM_BUFF)) > 0) {
                                for ( i = 0; i < bytes_read; ++i ) {
                                    ...
                                    validUpper=0;
                                    if(is_new_line && IS_UPPERCASE(primoCarattere))
                                        validUpper=1;
                                    ...
                                    if (c == '\n') {
                                        if (count == invioUDP.numOccorrenze && validUpper && valid ) {
                                            risultato++;
                                        }
                                        count = 0;
                                        is_new_line = 1;
