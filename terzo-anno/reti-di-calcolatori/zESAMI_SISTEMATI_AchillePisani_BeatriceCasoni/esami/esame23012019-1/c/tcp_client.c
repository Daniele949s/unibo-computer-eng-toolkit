...
int main(int argc, char *argv[]) {
    ...
    printf("Voglio ottenere la directory %s\n", input); //sarebbe num della targa
    if (write(sd, input, strlen(input)) < 0) {
        perror("Errore durante l'invio del nome della directory");
        exit(1);
    }
    ...
    if (nread <= 0) {
        perror("Errore durante la lettura della risposta del server");
        exit(1);
    }
    ...
    while (1) {
        ...
        if (read(sd, file_name, DIM_BUFF) <= 0) {
            perror("Errore durante la ricezione del nome del file");
            exit(1);
        }
        ...
        if (read(sd, &file_size, sizeof(int)) <= 0) {
            perror("Errore durante la ricezione della dimensione del file");
            exit(1);
        }
        ...
        while (bytes_left > 0) {
            ...
            if (bytes_read <= 0) {
                perror("Errore durante la ricezione dei dati del file");
                close(fd);
                exit(1);
            }
