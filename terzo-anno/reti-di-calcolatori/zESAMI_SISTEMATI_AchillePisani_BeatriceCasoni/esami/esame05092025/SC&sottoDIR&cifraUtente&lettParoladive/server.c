            ...
            if (pid > 0) {
               ...
               char cifra[2];
                ...
                while(1) {
                    ...
                    if (read(connfd, cifra, sizeof(cifra)) < 0) {
                        perror("read");
                        close(connfd);
                        exit(1);
                    }

                    printf("Servizio TCP: Direttorio richiesto: %s con cifra da controllare: %s\n", directory, cifra);
                    ...
                    // Directory esiste, la scorro
                        ...
                        while ((entry = readdir(dir)) != NULL) {
                            ...
                            if(entry->d_type != DT_DIR && !IS_TXT(entry->d_name)
                             && strcmp(entry->d_name + strlen(entry->d_name) -4 - strlen(cifra), cifra) == 0){
                                 //strncmp(entry -> d_name, prefisso, strlen(prefisso)) != 0
                                ...
                                snprintf(DIR_path, sizeof(DIR_path), "%s/%s", directory, entry -> d_name);
                                printf("Trovato DIR %s\n", DIR_path);

                                subdir = opendir(DIR_path);
                                if (subdir == NULL) {
                                    perror("opendir");
                                    continue;
                                }
                                 while ((subentry = readdir(subdir)) != NULL) {
                                    ...
                                    if(subentry->d_type != DT_DIR !IS_TXT(subentry->d_name)
                                    && strcmp(subentry->d_name + strlen(subentry->d_name) -4 - strlen(cifra), cifra) ==0){
                                        //strncmp(entry -> d_name, prefisso, strlen(prefisso)) != 0
                                        snprintf(file_path, sizeof(file_path), "%s/%s",DIR_path , subentry -> d_name);
                                        printf("Trovato file %s\n", file_path);
                            ...
                            while ((bytes_read = read(fd, buffer, DIM_BUFF)) > 0) {
                                for ( i = 0; i < bytes_read; ++i ) {
                                    ...
                                    if (c == '\n') {
                                        ...
                                        if ( strcmp(riga + strlen(riga) - strlen(invioUDP.parola), invioUDP.parola) == 0 && valid ) {
                                            risultato++;
                                        }
                                        // count = 0;
                                        // is_new_line = 1;
