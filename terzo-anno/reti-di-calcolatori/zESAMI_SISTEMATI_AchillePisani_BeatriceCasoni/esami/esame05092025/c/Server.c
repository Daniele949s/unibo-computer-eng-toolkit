        ...
        if (FD_ISSET(listenfd, &rset)) {
            ...
            if (pid > 0) {
                ...
                while(1) {
                    ...
                    //int len = read(connfd, numero, sizeof(numero));
                    if (read(connfd, numero, sizeof(numero)) < 0) {
                        perror("read");
                        close(connfd);
                        exit(1);
                    }

                    printf("Servizio TCP: Direttorio richiesto: %s, numero %s\n", directory, numero);
                    ...
                    // Directory esiste, la scorro
                        ...
                        while ((entry = readdir(dir)) != NULL) {
                            if(entry -> d_type == DT_DIR) // DA QUA IN POI è SOLO PER LE SOTTODIRECTORY DI PRIMO GRADO; FAI FIND (CTRL + F) CON "FINE SOTTODIRECTORI DI PRIMO GRADO" PER TROVARE DOVE FINISCE
                            {
                                dir2 = opendir(entry -> d_name);

                                while ((entry = readdir(dir2)) != NULL) {
                                    if (entry -> d_name[0] == '.' || ((strchr(NomeFile, '.') != NULL && strncmp((strchr(NomeFile, '.') - strlen(Stringa)), Stringa, strlen(Stringa)) == 0) || (strchr(NomeFile, '.') == NULL && strncmp((NomeFile + strlen(NomeFile) - strlen(Stringa)), Stringa, strlen(Stringa)) == 0)) ){
                                        continue;
                                    }
                            ...
                            } // FINE SOTTODIRECTORI DI PRIMO GRADO (ELIMINA QUESTA PARTE DA QUI A SOPRA PER CAPIRCI QUALCOSA)
                            else // ELSE: ALLORA SONO DEI FILE
                            {
                                if (entry -> d_name[0] == '.' || ((strchr(NomeFile, '.') != NULL && strncmp((strchr(NomeFile, '.') - strlen(Stringa)), Stringa, strlen(Stringa)) == 0) || (strchr(NomeFile, '.') == NULL && strncmp((NomeFile + strlen(NomeFile) - strlen(Stringa)), Stringa, strlen(Stringa)) == 0)) ){
                                    continue;
                                }
        ...
        if (FD_ISSET(udpfd, &rset)) {
            ...
            while(1){
                ...
                if (dir == NULL) {
                    ...
                    while ((entry = readdir(dir)) != NULL ) {
                        if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0 || !IS_TXT(entry->d_name)) {
                            continue;
                        }
                        ...
                        if (fd < 0) {
                            ...
                            while ((bytes_read = read(fd, buffer, DIM_BUFF)) > 0) {
                                for(i=0;i<bytes_read;++i) {
                                    ...
                                    /*if(c == '\n') {
                                        if(count == 4 && IS_UPPERCASE(primoCarattere) && valid ) {
                                            risultato++;
                                        }
                                        count = 0;
                                        //is_new_line = 1;
                                    }*/
                                    if(c == invioUDP.parola[count]) // Condizione sul carattere della riga
                                        count++;
                                    else if(c == '\n')
                                    {
                                        risultato++;
