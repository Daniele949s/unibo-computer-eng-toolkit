...
int main(int argc, char **argv)
{
    ...
    for (;;)
    {
                ...
                while(1) {
                    ...
                    // Directory esiste, la scorro
                        ...
                        while ((entry = readdir(dir)) != NULL) {
                            //qua cera la strcmp con il prefisso dell alro esercizio che ora non serve
                            if (entry -> d_name[0] == '.' || strcmp(entry->d_name, "..") == 0 ){
                                continue;
                            }

                            if(entry->d_type != DT_DIR && !IS_TEXT(entry->d_name)){
        ...
        if (FD_ISSET(udpfd, &rset)) {
            ...
            while(1){ //forse non ce bisogn qui perche non lo richiede ma lascio?
                ...
                printf("Servizio UDP: Messaggio ricevuto: %s %d\n", 
                    invioUDP.directory, 
                    //invioUDP.carattere, 
                ...
                if (dir == NULL) {
                    ...
                    while ((entry = readdir(dir)) != NULL ) {
                        ...
                        //ho strlen(entry->d_name) perche sizeof(entry->d_name) restituisce sempre 256, ma entry->d_name può contenere una stringa più corta.
                        ...
                        if (fd < 0) {
                            ...
                            while ((bytes_read = read(fd, buffer, DIM_BUFF)) > 0) {
                                for ( i = 0; i < bytes_read; ++i ) {
                                    ...
                                    if (c == '\n') {
                                        //count >= invioUDP.numOccorrenze && IS_UPPERCASE(primoCarattere)
                                        if(countLettere >= invioUDP.numOccorrenze && countNum >= invioUDP.numOccorrenze)
                                        {
                                            risultato++;
                                        }
                                        countLettere = 0;
                            ...
                            // Valuta anche l’ultima riga se non termina con \n
                            if (countLettere > 0 || countNum > 0) {
                                if (countLettere >= invioUDP.numOccorrenze && countNum >= invioUDP.numOccorrenze) {
                                    risultato++;
                                }
                            }
