...
public class RMI_Server extends UnicastRemoteObject implements RMI_interfaceFile {
	...
	@Override
	public int elimina_parola(String filename, String parola) throws RemoteException, FileNotFoundException {
		System.out.println("Metodo elimina_parola invocato..." + filename + parola);
		// Variabili --------------------------------------------------------
		int i, result = 0;
				...
				try (BufferedWriter writer = new BufferedWriter(new FileWriter(tempFile))) {
					...
					while ((line = reader.readLine()) != null) {
						...
						for(int j = 0; j<testo.length;j++){
							...
							if (!testo[j].equals(parola)) { // Condizione di scrittura
								writer.write(testo[j]);
								writer.write(" ");
							} else {
								result ++;
							}
	...
	/* ------------------------------------------------------------------- */
	/* SERVIZIO 2 ======================================================== */
	//file testo dim <, binario>soglia
	@Override
	public String[] lista_nomifile_soglia(String dirName, int soglia) throws RemoteException, FileNotFoundException {
		System.out.println("Metodo lista_nomifile_soglia invocato...");
		// Variabili --------------------------------------------------------
		...
		if (directory.exists() && directory.isDirectory()) {
			...
			for ( i = 0; i < files.length && arraySize < MAX_LIST_SIZE; i++) {
				...
				if (file.isFile()) {
					...
					if (file.getName().endsWith(".txt") && file.length() < soglia) {
						arraySize++;
					}
					if (!file.getName().endsWith(".txt") && file.length() > soglia) {
						arraySize++;
